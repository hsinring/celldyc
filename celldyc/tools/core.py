...
def calculate_magnitude_scale(
    adata: AnnData,
    percentile: float = 95.0,
    layer: str = "velocity",
    output_key: str = "magnitude_scale",
) -> AnnData:
    """
    Computes the gene-level velocity magnitude scale for a given gene.
    ...
    percentile : float, optional
        Percentile threshold for selecting high-velocity values.
        Default is 95.0.
    layer : str, optional
        Layer containing velocity data. Default is 'velocity'.
    output_key : str, optional
        Column name in var for storing scaling factors.
        Default is 'magnitude_scale'.

    Returns
    -------
    AnnData
        Updated AnnData object with scaling factors in var[output_key].

    Raises
    ------
    ValueError
        If required layers are not found.
    ...
    mag = get_magnitude(adata)
    vel = adata.layers[layer].copy()

    if hasattr(mag, "toarray"):
        mag = mag.toarray()
    ...
    for i in range(adata.n_vars):
        gene_vel = np.abs(vel[:, i])
        threshold = np.percentile(gene_vel, percentile)
        top_vel = gene_vel[gene_vel >= threshold]

        if len(top_vel) > 0:
            top_avg = np.mean(top_vel)
        else:
            top_avg = 0.0

        gene_top_avg.append(top_avg)

    gene_top_avg = np.array(gene_top_avg)
    gene_means = np.mean(mag, axis=0)

    scale_vals = np.where(gene_top_avg > 0, gene_means / gene_top_avg, np.nan)
    adata.var[output_key] = scale_vals
    print(f"Scale factors stored in adata.var['{output_key}']")
